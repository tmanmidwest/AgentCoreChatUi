#!/bin/bash
# =============================================================================
# restore-state.sh — Rebuild .agentcore-chat-state from live AWS resources
# =============================================================================
...
#
# It discovers every resource by the naming convention deploy.sh uses,
# then writes a .agentcore-chat-state file identical to what deploy.sh produces.
# It creates nothing in AWS — read-only discovery.
#
# Note: agent ARN / ID / alias are re-prompted because they are not stored in
# AWS resource names and cannot be rediscovered automatically. Have them ready.
#
# Usage:
...
# ── AGENT CONFIGURATION (cannot be discovered — must be re-entered) ───────────
header "Agent configuration"

echo ""
echo -e "  ${BOLD}Agent details cannot be rediscovered from AWS.${NC}"
echo -e "  Please re-enter them. Find them in: Bedrock → Agents → your agent → Aliases"
echo ""
...
success "Agent configuration accepted"
...
if aws ecr describe-repositories --repository-names "${APP_NAME}-webapp" \
  ...
  success "ECR image: $CONTAINER_IMAGE"
fi
...
[ -n "$SECRET_ARN" ] && success "Secret: $SECRET_ARN" || warn "Secrets Manager secret not found"
...
if [ -z "$ALB_ARN" ] || [ -z "$TG_ARN" ]; then
  ...
  region. Re-run with the region you deployed to:  ./restore-state.sh us-west-2"
fi
...
cat > "$STATE_FILE" <<EOF
# agentcore-chat deployment state — regenerated by restore-state.sh
# Rediscovered from live AWS resources; not the original deploy.sh output.
