#!/bin/bash
# =============================================================================
# manage.sh — agentcore-chat Day-to-Day Management
# =============================================================================
# Usage:
#   ./manage.sh status      — show current app status and URL
#   ./manage.sh stop        — pause the app (no Fargate charges, data kept)
...
#   ./manage.sh config      — show the current agent + app configuration
# =============================================================================
...
case "$CMD" in
  ...
  status)
    echo ""
    echo -e "${BOLD}  ${APP_DISPLAY_NAME:-agentcore-chat} — Status${NC}"
    echo -e "  ─────────────────────────────────────────"
  ...
  stop)
    echo ""
    log "Stopping ${APP_DISPLAY_NAME:-agentcore-chat} (setting desired count to 0)..."
  ...
  start)
    echo ""
    log "Starting ${APP_DISPLAY_NAME:-agentcore-chat}..."
    aws ecs update-service \
      --cluster "$APP_NAME" \
      --service "${APP_NAME}-webapp" \
      --desired-count 1 \
  ...
  config)
    echo ""
    echo -e "${BOLD}  ${APP_DISPLAY_NAME:-agentcore-chat} — Configuration${NC}"
    echo -e "  ─────────────────────────────────────────"
    echo ""
    echo -e "  ${BOLD}App display name:${NC}  ${APP_DISPLAY_NAME:-not set}"
    echo -e "  ${BOLD}Region:${NC}            $REGION"
    echo -e "  ${BOLD}Agent ARN:${NC}         ${AGENT_ARN:-not set}"
    echo -e "  ${BOLD}Agent ID:${NC}          ${AGENT_ID:-not set}"
    echo -e "  ${BOLD}Agent Alias ID:${NC}    ${AGENT_ALIAS_ID:-not set}"
    echo -e "  ${BOLD}Agent region:${NC}      ${AGENT_REGION:-not set}"
    echo -e "  ${BOLD}Secrets:${NC}           ${SECRET_ARN:-not set}"
    echo ""
    echo -e "  To change any of these, update the secret in AWS Secrets Manager"
    echo -e "  and run ${BOLD}./manage.sh restart${NC} to pick up the new values."
    echo ""
    ;;
  ...
  help|*)
    echo ""
    echo -e "${BOLD}  ${APP_DISPLAY_NAME:-agentcore-chat} — Management Commands${NC}"
    echo ""
    ...
    echo -e "  ${BOLD}./manage.sh config${NC}   Show agent and app configuration"
    echo ""
    echo -e "  ${BOLD}./deploy.sh${NC}          Deploy everything from scratch"
    echo -e "  ${BOLD}./update.sh${NC}          Rebuild and redeploy from latest source"
