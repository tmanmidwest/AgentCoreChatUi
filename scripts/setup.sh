#!/bin/bash
# =============================================================================
# setup.sh — Pre-deployment prerequisite checker for agentcore-chat
# =============================================================================
...
echo ""
echo -e "${BOLD}${BLUE}  agentcore-chat — Setup Checker${NC}"
...
if command -v node >/dev/null 2>&1; then
  ...
  fail "Node.js not found (required to build the frontend)"
...
if [ -n "$CALLER" ]; then
  ...
  info "Run 'aws configure' and enter your Access Key ID, Secret Access Key, and region"
fi
...
if [ -n "$VPC_ID" ] && [ "$VPC_ID" != "None" ]; then
  ...
  info "Create one in the AWS console: EC2 → Your VPCs → Actions → Create Default VPC"
fi

# ── AGENT CONNECTIVITY ────────────────────────────────────────────────────────
header "AWS Bedrock AgentCore"

info "You will be prompted for your Agent ARN, Agent ID, and Agent Alias ID during deploy."
info "Find these in the AWS console under: Bedrock → Agents → your agent → Aliases"

# Test Bedrock access if we have a region
if [ -n "${REGION:-}" ]; then
...
if [ "$FAILED" -eq 0 ]; then
  ...
  echo ""
  echo -e "  ${YELLOW}Have these values ready — deploy.sh will ask for them:${NC}"
  echo -e "    • AWS Bedrock Agent ARN"
  echo -e "    • Agent ID"
  echo -e "    • Agent Alias ID"
  echo -e "    • A name for your app (shown in the chat UI)"
else
